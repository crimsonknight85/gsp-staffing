-- =============================================================
-- GSP Platform — Phase 2 Database Schema
...
-- Users can see orgs they belong to; admins see all
...
-- Users can see their own memberships; org members can see memberships in their org;
-- admins see all
...
-- Only admins and HR can create/remove memberships
drop policy if exists "Memberships: admin/hr insert" on public.organization_memberships;
create policy "Memberships: admin/hr insert"
  on public.organization_memberships for insert
  with check (
    public.is_admin()
    or (
      public.get_user_role(organization_id) = 'hr'
      and organization_id in (
        select id from organizations where type = 'gsp'
      )
    )
  );
...
-- ── Clients policies ───────────────────────────────────────
-- Users in the same org can read their client record; admins and HR can read all
drop policy if exists "Clients: read org member or admin/hr" on public.clients;
create policy "Clients: read org member or admin/hr"
...
-- Only admins can write client records
...
-- Users can read audit events for orgs they belong to; admins see all
...
-- Only the system (service role) or admins can insert audit events
